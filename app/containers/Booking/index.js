...
export default class Booking extends React.PureComponent {
  ...
  removeTime = (day, time, j) => {
    ...
    for (let i = 0; i < activeTimes.length; i++) {
      if (
        activeTimes[i].day === sched.day &&
        activeTimes[i].time === sched.time
      ) {
  ...
  renderTypeButton = (res, i) => {
    if (this.state.activeType === res.id) {
      return (
        <div
          className="bookingActiveTypeButton"
          key={`activeButton${i}`}
          onClick={() => this.handleType(res.id)}
        >
          {res.resourceName}
        </div>
      );
      ...
      return (
        <div className="bookingTimeBlock" key={`bookingTimeBlock2${j}`}>
          ...
          <div className="bookingTimeButton">
            <FlatButton
              style={{ background: "#ff4d58", color: "#FFFFFF" }}
              onClick={() => this.removeTime(day, item.time, j)}
            >
              Remove
            </FlatButton>
          </div>
  ...
  testSlot = (start, end, event) => {
    ...
    if (minutes > this.state.increment) {
      this.showSnack(
        "This can only be booked for " + this.state.increment + " minutes."
      );
    } else {
      let dateObject = {
  ...
  render() {
    return (
      <div className="bookingContainer">
        <Helmet
          title="Bookings"
          meta={[{ name: "description", content: "Book a Time Slot" }]}
        />
        ...
        <main className="bookingMainContainer">
          <div className="bookingInfoContainer">
            <h3 className="bookingColumnTitle">Your Info</h3>
            <TextField
              label="Your Name"
              margin="normal"
              fullWidth={true}
          ...
          <div
            style={{ width: "100%", background: "#FFFFFF", padding: "10px" }}
          >
            {/*<BigCalendar
                            selectable
