...
export default class SpaceSignUp extends React.PureComponent {
  ...
  handleAddress = event => {
    this.setState({ address: event.target.value.replace(/\s\s+/g, " ") });
  };
  handleState = event => {
  ...
  handleDescription = editorState => {
    this.setState({ description: editorState, editorState: editorState });
  };
  handleLogo = event => {
    event.preventDefault();
  ...
  storeSpace = () => {
    ...
    data.append(
      "description",
      drafToHtml(convertToRaw(this.state.description.getCurrentContent()))
    );
    ...
    fetch("https://innovationmesh.com/api/workspaces", {
      ...
      .then(spaceID => {
        if (spaceID.error) {
          ...
          this.showSnack(
            "Thanks! We will review your workspace and be in contact soon."
          );
          setTimeout(() => {
  ...
  renderAvatarImageText = () => {
    if (
      ...
      this.state.avatarPreview === undefined ||
    ...
    return (
      <div className="spaceSignUpcontainer">
        <Helmet>
          ...
          <meta
            name="description"
            content="Description of cowork space signup form"
          />
        </Helmet>
        ...
        <main className="spaceSignUpMain">
          ...
          <div className="spaceSignUpContainer">
            ...
            <Link to={"/spaces"} style={{ alignSelf: "center", width: "80%" }}>
              <FlatButton
                style={{
                  width: "100%",
              ...
              >
                Not a Founder? Join a WorkSpace instead!
              </FlatButton>
            </Link>
          </div>
        </main>
